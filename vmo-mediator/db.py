...
def initialize_database(dbname="database.sqlite"):
    ...
    sql_create_user_table = """ CREATE TABLE IF NOT EXISTS users (
                                        ...
                                        CallHandlerObjectId text NOT NULL,
                                        AlternateGreetingEnabled text NOT NULL,
                                        Active text NOT NULL
                                    ); """


    try:
        conn.execute(sql_create_user_table)
    except sqlite3.Error as e:
        print("Error: "+e)
        return(False)
    conn.commit()
...
def insert_into_database(dbname,table,**kwargs):
    ...
    insert="INSERT INTO "+table+" (ID,"+fieldnames+") VALUES ('"+id+"',"+values+")"
    print(insert)
    try:
        conn.execute(insert)
        conn.commit()
